...
namespace Infrastructure.Identity.JwtBearer
{
    ...
    /// <see cref="JwtBearerHandler"/> for handling sign in, sign out, and generating JWT tokens.
    /// </summary>
    public class CustomJwtBearerHandler : SignInAuthenticationHandler<CustomJwtBearerOptions>
    {
        [Obsolete("ISystemClock is obsolete, use TimeProvider on AuthenticationSchemeOptions instead.")]
        public CustomJwtBearerHandler(IOptionsMonitor<CustomJwtBearerOptions> options, 
        ...
        protected override async Task HandleSignInAsync(ClaimsPrincipal user, AuthenticationProperties? properties)
        {
            ...
            {
                AccessToken = CreateBearerToken(user, utcNow),
                ExpiresIn = (long)Options.BearerTokenExpiration.TotalSeconds,
                RefreshToken = CreateRefreshToken(user, utcNow),
            };
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private string CreateBearerToken(ClaimsPrincipal user, DateTime utcNow)
        {
            var expires = utcNow + Options.BearerTokenExpiration;
            ...
            // Generates token with the above information
            var tokenHandler = new JwtSecurityTokenHandler();
            var token = tokenHandler.CreateToken(tokenDescriptor);
            return tokenHandler.WriteToken(token);
        }

        private string CreateRefreshToken(ClaimsPrincipal user, DateTime utcNow)
        {
            ...
            var ticket = new AuthenticationTicket(user, refreshProperties, $"{Scheme.Name}:RefreshToken");
